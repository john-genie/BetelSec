...
const riskBriefingPrompt = ai.definePrompt({
  ...
  prompt: `
    ...
    The client's company is '{{{companyName}}}'. 
    They are in the '{{{industry}}}' industry.
    Their company size is '{{{enterpriseSize}}}'.
    Here is a description of their business: {{{companyDescription}}}

    Based on all this information, generate the following sections, ensuring each point is directly relevant and personalized to {{{companyName}}}. Use markdown for formatting, especially bullet points (*).

    1.  **Sensitive Data Profile for {{{companyName}}}:** Based on their industry and business description, identify and list the most critical sensitive data types that an organization like {{{companyName}}} handles. Be specific.

    2.  **Threat Analysis & Real-World Impact for the {{{industry}}} Sector:**
        *   Describe the most likely quantum and classical cyber threats that a company like {{{companyName}}} (a '{{{enterpriseSize}}}' size business in this industry) faces. Examples include "Harvest Now, Decrypt Later," state-sponsored espionage, and specific types of ransomware.
        *   Comment on how frequently companies in this sector are targeted.
        *   Provide a specific, real-world example of a major cyberattack that has recently impacted the '{{{industry}}}' sector. You MUST use a well-known incident (e.g., Change Healthcare for Healthcare, CDK Global for automotive, etc.). Describe the attack and, most importantly, state the financial loss, ensuring it is a multi-million dollar figure to underscore the severity.

    3.  **Recommended BetelSec Solutions for {{{companyName}}}:** Recommend specific BetelSec products based on the company's profile and the threats identified. Personalize the explanation for {{{companyName}}}.
